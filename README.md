...
LINE 傳「P華新」「P1605」「PCSF」→ 回覆保證金級距、依結算價的正式保證金、依即時價的盤中估算。ETF 期貨直接顯示期交所公告的固定保證金金額；處置中契約會加註警示。**只有 `P` 開頭的訊息會觸發回覆**（不分大小寫、容許全形；亦相容舊的 `P/` 斜線格式），其他訊息一律不回應。
